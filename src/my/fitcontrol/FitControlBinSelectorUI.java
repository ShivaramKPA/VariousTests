...
import static my.fitcontrol.Constants.thEdgeVzH;
import static my.fitcontrol.Constants.thEdgeVzL;
 ...
 * @author kpadhikari
 */
public class FitControlBinSelectorUI extends JFrame {
    ...
    private JCheckBox[] checkBoxes4AngleBins = new JCheckBox[17];
    ...
    public FitControlBinSelectorUI() {
        initLookAndFeel();
    ...
    public void createTopPanelForParameterForm() {
        ...
        int ii=0, iii = 0;
        for (int i = 0; i < 7; i++) {
            //First row of check boxes will have 7 check-boxes (from 0 to 6)
            c.gridx = i;
            c.gridy = 0;
            c.anchor = GridBagConstraints.LINE_START; //kp: to align components to the left side of grid cells.        
            checkBoxes4AngleBins[i] = new JCheckBox(String.format("(%2.1f,%2.1f)",thEdgeVzL[i], thEdgeVzH[i]));
            panelForm.add(checkBoxes4AngleBins[i], c);
            
            //Now the second row (will contain only 3 checkboxes)
            if (i < 3) {
                ii = i + 7;
                c.gridx = 2+i;
                c.gridy = 1;
                c.anchor = GridBagConstraints.LINE_START; //kp: to align components to the left side of grid cells.        
                checkBoxes4AngleBins[ii] = new JCheckBox(String.format("(%2.1f,%2.1f)", thEdgeVzL[ii], thEdgeVzH[ii]));
                panelForm.add(checkBoxes4AngleBins[ii], c);
            }
            
            //Now the third row (will contain 7 checkboxes
            iii = i + 3 + 7; //I will put one check boxes in the middle row 
            c.gridx = i;
            c.gridy = 2;
            c.anchor = GridBagConstraints.LINE_START; //kp: to align components to the left side of grid cells.        
            checkBoxes4AngleBins[iii] = new JCheckBox(String.format("(%2.1f,%2.1f)",thEdgeVzL[iii], thEdgeVzH[iii]));
            panelForm.add(checkBoxes4AngleBins[iii], c);            
        }
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            new FitControlBinSelectorUI().setVisible(true);
        });
    }
}
