...
public class BasicDemo2verticalLineAdded {
	...
	public static void main(String[] args) {
		...
		GraphErrors [] verticalLine = new GraphErrors [2];
                verticalLine[0] = new GraphErrors();
                verticalLine[1] = new GraphErrors();
                for (int i = 0; i < 10; i++) {
                    verticalLine[0].addPoint(4.9, i*6, 0, 0);
                    verticalLine[1].addPoint(5.3, i*6, 0, 0);
                }
		...
		double yMax = canvas.getPad(0).getAxisY().getRange().getMax();
                System.out.println("pad(yMin, yMax): " + yMin + " " + yMax);//Returned 0.0 & 2.0
                
                //Making a graph of a single point  with a huge error bar 
                //In effect, using y-error bar of the single point to draw a vertical line
                //
		//line.addPoint(4.4, (yMax-yMin)/2.0, 0, (yMax-yMin)/2.0); 
		line.addPoint(4.4, 50.0, 0, 50.0); //addPoint(x,y,xerr,yerr)
		...
		verticalLine[0].setMarkerColor(3);
		verticalLine[0].setMarkerSize(3);
		verticalLine[0].setLineColor(3);
		verticalLine[1].setMarkerColor(2);
		verticalLine[1].setMarkerSize(0);
		verticalLine[1].setLineColor(2);
		
		canvas.draw(barber);
		canvas.draw(bodemkamp,"same");
                canvas.draw(verticalLine[0],"same");
                canvas.draw(verticalLine[1],"same");
